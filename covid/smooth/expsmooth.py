...
class ExpSmooth(nn.Module):
    def __init__(self,
        ...
        self.register_buffer('_levels', None, persistent=False)
        self.register_buffer('_trends', None, persistent=False)
        self.register_buffer('_season', None, persistent=False)

    @property
    def level_diffs(self) -> Tensor:
        return pt.abs(pt.cat([self._individual_params[:,:1], self._levels[:,:-1]], dim=1) - self._levels).mean(dim=1)
