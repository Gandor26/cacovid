...
class XSeriesAttention(nn.Module):
    def __init__(self,
        ...
        self.register_buffer('local_score', None, persistent=False)
        self.register_buffer('ref_score', None, persistent=False)
        self.register_buffer('q_scale', None, persistent=False)
    ...
    def minmax_conv1d(
        ...
        feats: Optional[Tensor] = None,     # [N, F, T]
    ...
    def _compute_ar(self, query: Tensor) -> Tensor:
    ...
    def _compute_scores(self,
        ...
        if r is None:
            ...
            r_score = pt.sum(q * (self.r_key if self.fix_ar_key else q), dim=-1, keepdim=True)
            
        if self.add_autoreg:
            m_score = pt.sum(q * (self.m_key if self.fix_ar_key else q), dim=-1, keepdim=True)
        else:
            m_score = None
        ...
        for skip in range(self.n_rolls):
            ...
            if self.full_attention:
                pr = self.fully_soft_attention(
                    score, value, r_score, r, m_score, m,
                )
            else:
